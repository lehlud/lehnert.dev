...
class Blog
{
    ...
    {
        return file_get_contents(__DIR__."/../www/blog/{$this->id}/_transcript.html");
    }
    ...
    public static function get(string $id): Blog|null
    {
        ...
        $path = __DIR__ . "/../www/blog/$id/_definition.json";
        if (!file_exists($path))
            return null;
    ...
    {
        $dirs = scandir(__DIR__ . '/../www/blog');
        $dirs = array_filter($dirs, fn($dir) => is_dir(__DIR__ . "/../www/blog/$dir") && $dir !== '.' && $dir !== '..');
